...
export default function OfficerSuspectedList() {
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      if (response.ok) {
        // Only keep transactions that are flagged
  ...
  // Remove flag (unflag) a transaction
  const handleRemoveFlag = async (transactionId) => {
    const token = localStorage.getItem("token");
    try {
      const response = await fetch(`${API_BASE_URL}/bank/transactions/${transactionId}/unflag`, {
        method: "PUT",
        headers: {
      ...
      if (response.ok) {
        ...
        alert(data.message || "Failed to unflag transaction.");
      }
    } catch (err) {
  ...
  return (
    <main className="p-6 mt-24 bg-gray-100 min-h-screen">
      {/* Header Section: Title on Left, Search Inputs on Right */}
            ...
            <tbody>
              {filteredTransactions.map((txn) => (
                <tr key={txn.transactionId} className="text-center">
                  ...
                  <td className="px-4 py-2 border">
                    <button
                      onClick={() => handleRemoveFlag(txn.transactionId)}
        ...
        <div className="fixed inset-0 flex items-center justify-center bg-black bg-opacity-50 z-50">
          <div className="bg-white rounded-lg shadow-lg w-full max-w-lg p-6">
            ...
            <div className="space-y-2">
              ...
              <p>
                <strong>Amount:</strong> ${selectedTransaction.amount || "N/A"}
              </p>
            </div>
